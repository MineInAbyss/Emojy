...
class EmojyNMSHandler : IEmojyNMSHandler {
    ...
    private data class PlayerHandler(val player: Player) {
        ...
        private fun writeNewNbt(originalBuffer: EmojyBuffer, clientBound: Boolean): EmojyBuffer {
            val newBuffer = EmojyBuffer(Unpooled.buffer())
            val id = VarInt.read(originalBuffer)
            VarInt.write(newBuffer, id)

            val bytes = byteArrayOf(originalBuffer.readableBytes().toByte())
            originalBuffer.readBytes(bytes)
                    ...
                    if (TagTypes.getType(aByte.toInt()) === CompoundTag.TYPE) {
                        ...
                        runCatching {
                            DataInputStream(ByteArrayInputStream(nbtByte)).use { input ->
                                ...
                                val oldComponent = PaperAdventure.asAdventure(
                                    ComponentSerialization.CODEC.decode(NbtOps.INSTANCE, getTag).getOrThrow().first
                                )
                                ...
                                val newTag = ComponentSerialization.CODEC.encode(
                                ...
                                DataOutputStream(stream).use { output ->
                                    ...
                                    for (b in stream.toByteArray()) list += b
                                }
                            }
                        }
                    }
